...
ExportableModels = namedtuple('ExportableModels', ['model'])
...
def prepare_for_export() -> List[ExportableModels]:
    logging.info('collecting models for export')
    props = get_props()
    world_objects = [ExportableModels(prop_object) for prop_object in props]
...
def export(models : ExportableModels, output_path : str) -> None:
    exportable_models = [single_model.model for single_model in models]
